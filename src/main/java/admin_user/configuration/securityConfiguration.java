...
public class securityConfiguration {
	...
	@Autowired
	CustomSuccessHandler customSucessHandler;
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception
	{
		...
		.authorizeHttpRequests(request -> request.requestMatchers("/admin-page")
				...
				.requestMatchers("/inscription", "/css/**").permitAll()
				.anyRequest().authenticated())
		
		.formLogin(form -> form.loginPage("/connexion").loginProcessingUrl("/connexion")
				.successHandler(customSucessHandler).permitAll())
		
		
		.logout(form -> form.invalidateHttpSession(true).clearAuthentication(true)
				.logoutRequestMatcher(new AntPathRequestMatcher("/deconnexion"))
				.logoutSuccessUrl("/connexion?deconnexion").permitAll())
		;
